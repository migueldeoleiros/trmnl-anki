...
- `GET /api/current?query=...` uses a raw Anki search; `GET /api/current?deck=...&filter=...` composes `deck:"..." (<filter>)`.
- Optional `cadence_minutes` accepts `15`, `30`, or `60`; cold custom queries return `status: not_ready` while the background worker fills that cache entry.
